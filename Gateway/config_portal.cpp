 ...
 *
 * 工作流程 (参考项目 SmartConfig 替代方案):
 *   1. 首次启动: Flash 无 /wifi.cfg → 自动进入配网模式
 *   2. 配网模式: softAP("ParkingGateway_Config") + DNS 劫持
 *      → 手机连热点 → 浏览器访问任意域名/192.168.4.1
 *      → 页面显示扫描到的 WiFi 列表 + 密码输入 → 保存
 *   3. 保存成功: 写入 LittleFS → ESP.restart() 用新配置连接
 *   4. 运行中: 长按 CONFIG_KEY_PIN 5s → 重新进入配网模式
 *
...
/* ==================== WiFi 配置持久化 ==================== */
#define CFG_MAGIC   0x5A  /* 配置文件有效性标记 */
...
/* 主页: WiFi 列表 + 密码表单 + 实时状态区
 * 所有中文均为 HTML 实体, 保证任意编码下正常显示 */
...
/* 状态 JSON: MQTT/节点/内存 (纯 ASCII, 中文用 HTML 实体) */
...
{
    /* 进入 AP+STA 模式: AP 供手机连接, STA 用于扫描 WiFi */
