...
> **实施状态（2026-09-10 修订）**：本文第 1 节为**历史问题分析**，所述缺陷多已修复落地，勿据此判定当前系统状态：
> - 两阶段提交（先验固件头再擦 APP）、失败清标志安全回退、Boot 升级失败不再裸 `while(1)`（改为持续监听按键/`AT+OTA` 可救活）均已实现；
> - 触发握手、指数退避重传、断点续传（每 32 包）、整链重试、自愈预算 3 次等已随 S24–S33 于 2026-09-09/10 落地并编译验证（见 [通信架构重构方案.md](通信架构重构方案.md) 第 15 节）；
> - **断电自愈**：已通过"节点来电回 `PONG,BOOT` → 网关 `ota_autoDispatch()` 自动续传（固件保留至 `PONG,APP`，自愈预算 3 次）"实现，无需人工介入；`BOOT_READY 0x42` + `OTA_SUSPEND` 为早期设计稿，已被该机制替代（仅作"主动上报加速恢复"增强参考），详见 [技术待办-延后事项.md](技术待办-延后事项.md) 现状核实。
...
- 3 次全部失败回 `OTA_IDLE` 并向平台上报失败（`onenet_ota.cpp` 上报 202）
- 平台侧 `OTA_PLAT_FINISH` 的判定从"网关一次性结果"改为"网关整链重试最终结果"

> **已实施变更（P0-3，2026-09-09）**：重试耗尽**不再删除固件文件**。原设计"重试耗尽删文件回 `OTA_IDLE`"会掐断自愈通道——节点可救（如拔电残留 APP_PARTIAL 导致写 PGERR）却因文件被删而永久僵持。现改为：重试耗尽回 `OTA_IDLE`、上报 202，但**固件文件保留**，节点后续上报 `PONG,BOOT` 时统一判定器（`ota_autoDispatch`）自动重发自愈。文件仅在 (a) 节点确认离开 Boot（`PONG,APP`）或 (b) 被新固件覆盖时清理。平台账目不受影响：202 上报不依赖删文件。
